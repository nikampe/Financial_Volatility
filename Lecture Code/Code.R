...
Box.test(lsp500,lag=30,type="Ljung") #Ljung-Box statistic Q(5)
...
Box.test(a$residuals,lag=10,type="Ljung") #Ljung-Box statistic Q(5)Box.test
...
1/sqrt(length(ibm))*1/sqrt(var(ibm^2))*(max(a)-min(a)) #R/S statistic with sample variance


1/sqrt(length(ibm))*1/sqrt(lrvar(ibm^2,type="Newey-West")*length(ibm^2))*(max(a)-min(a)) #R/S statistic with HAC variance
...
b=lm(rv.proxy~s$my.sigmasq) #Univariate in-sample MZ test:
summary(b)
vcov(b) #OLE standard errors
hccm(b,type="hc0") #White consistent standard errors

b=lm((rv.proxy/s$my.sigmasq)[2:n]~(rv.proxy/s$my.sigmasq)[1:(n-1)]) #Alternative test 
summary(b)
